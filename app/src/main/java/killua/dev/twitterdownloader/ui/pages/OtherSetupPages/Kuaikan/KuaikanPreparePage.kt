package killua.dev.twitterdownloader.ui.pages.OtherSetupPages.Kuaikan
...
fun KuaikanPreparePage() {
    ...
    val loginState = viewModel.kuaikanLoginState.collectAsStateWithLifecycle()
    val eligibility = viewModel.kuaikanEligibility.collectAsStateWithLifecycle()
    LaunchedEffect(Unit) {
    ...
    SetupScaffold(
        ...
        if(isShowReset){
            CancellableAlert(
                title = "Reset now?",
                mainText = "Your login information will be cleared, and you will need to log in again to continue using Kuaikan's functions",
        ...
        Column(
            ...
            Column(
                ...
                AppIcon(AvailablePlatforms.Kuaikan)
                HeadlineMediumText(modifier = Modifier.paddingTop(SizeTokens.Level12), text = "Kuaikan Configurations")
            }

            Section(title = "Log in to your Kuaikan account") {
                ClickableConfigurationButton(
                    title = "Log in",
                    description = "Your Kuaikan's cookie is necessary when downloading pictures from it.",
            ...
            Section(title = "Log out") {
                ClickableConfigurationButton(
                    title = "Log out",
                    description = "Clear your Kuaikan account's cookie",
