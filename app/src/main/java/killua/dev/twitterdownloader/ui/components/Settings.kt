...
import killua.dev.twitterdownloader.core.utils.withState
import killua.dev.twitterdownloader.ui.animations.AnimatedTextContainer
...
fun Clickable(
    ...
    Clickable(enabled = enabled, desc = desc, onClick = onClick, indication = if (readOnly) null else ripple()) {
        Row(modifier = Modifier.height(IntrinsicSize.Min), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(
            SizeTokens.Level16)) {
